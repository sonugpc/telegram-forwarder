...
function saveRoutesConfig(routes) {
  const configPath = path.resolve(
    process.env.ROUTES_CONFIG_PATH || "./config/routes.json",
  );
...
function createServer(state) {
  ...
  app.post("/api/auth/:type", requireAuth, (req, res) => {
    ...
    try {
      ...
      const newRoute = {
        ...
        filters: {
          ...
          allowedMediaTypes: body.filters?.allowedMediaTypes || [
            "photo",
            "video",
            "document",
            "audio",
  ...
  app.put("/api/routes/:id", requireAuth, (req, res) => {
    try {
      ...
      if (idx === -1) {
        return res
          .status(404)
          .json({ success: false, error: `Route "${id}" not found` });
      }
   ...
   */
  app.post("/routes/reload", requireAuth, (req, res) => {
    try {
      const configPath =
        process.env.ROUTES_CONFIG_PATH || "./config/routes.json";
      const { routes, sourceMap } = loadConfig(configPath);
...
function startServer(state) {
  ...
  server.on("error", (err) => {
    if (err.code === "EADDRINUSE") {
      logger.error(
        `[Server] Port ${port} is already in use. Set a different PORT in .env`,
      );
    } else {
