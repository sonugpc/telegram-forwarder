...
function isRouteActive(schedule) {
    ...
    try {
        ...
        logger.warn(`[Handler] Invalid timezone "${schedule.timezone}", using Local time`);
        timeString = new Intl.DateTimeFormat('en-CA', { hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }).format(new Date());
    }
...
async function handleMessageWithMap(client, sourceMap, event) {
    ...
    if (!channelId) {
        logger.debug(`[Handler] Could not resolve channel ID for message ${message.id}`);
        return;
    }
    ...
    for (const route of routes) {
        try {
            logger.info(`[Handler] Processing route "${route.id}" (${route.name || 'unnamed'})`);
            ...
            if (route.processing?.enabled && route.processing?.url) {
                ...
                if (result.skip) {
                    logger.info(`[Handler] Route "${route.id}" — message skipped by processor`);
                    continue;
                }
            ...
            await forwardMessage(client, route, message, processedText, mediaType);
        } catch (err) {
            logger.error(
...
function registerHandler(client, sourceMap) {
    ...
    logger.info(`[Handler] ✅ Message handler registered — listening for new messages`);
}
